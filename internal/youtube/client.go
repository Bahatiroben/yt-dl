...
func (c *Client) GetVideo(url string) (*Video, error) {
    ...
    // Fetch watch page with better headers
...
// fetchWatchPage with realistic browser headers
func (c *Client) fetchWatchPage(videoID string) (string, error) {
    ...
    // Important: Make it look like a real browser
    ...
    if resp.StatusCode != http.StatusOK {
        return "", fmt.Errorf("unexpected status code: %d (possible bot detection)", resp.StatusCode)
    }
...
// Download method (unchanged for now)
func (c *Client) Download(video *Video, outputPath string) error {
    ...
    if format.URL == "" {
        return fmt.Errorf("selected format requires deciphering (not implemented yet)")
    }
